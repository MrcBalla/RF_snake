...
class BaseEnvironment:
    ...
    def __init__(self, n_boards, board_size):
        ...
        self.boards = np.ones((self.n_boards, self.board_size, self.board_size)) * self.EMPTY  # tensor filled with ones, the board is a matrix of ones repeated the number of board times
        # store the bodies of the snakes
        self.bodies = [[] for _ in range(self.n_boards)] # number of snake that are running
...
class OriginalSnakeEnvironment(BaseEnvironment):
    def __init__(self, n_boards, board_size):
        ...
        # boards
        self.boards[:, [0, -1], :] = self.WALL # define the wall as the boarder 
        self.boards[:, :, [0, -1]] = self.WALL
        for board in self.boards:
            # add head, add fruit
            available = np.argwhere(board == self.EMPTY)
            ind = available[np.random.choice(range(len(available)))]
            board[ind[0], ind[1]] = self.HEAD  # coordinate of the head generated randomly form all the available position
            available = np.argwhere(board == self.EMPTY)
            ind = available[np.random.choice(range(len(available)))]
            board[ind[0], ind[1]] = self.FRUIT  # position the fruit as before
