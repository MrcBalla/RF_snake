...
# change reward function to eploit other effect on prediction
    ...
    def act(self, letter: str):
        ...
        :return: (reward, done)
        """
        if letter in self.already_chosen_letters: # check if already proposed
            raise Exception("already chosen")
        if letter not in self.only_letters_list:  # check if the letter is in langauge dictionary
        ...
        if len(set(list(self.current_word_remaining))) == 1:
            self.reset()
            return 1, 1  #your done 

        return 0, 0  # next turn
